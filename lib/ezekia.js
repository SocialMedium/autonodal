...
async function getPeople(options = {}) {
  ...
  // Ezekia quirk: fields[] strips base-level data (name, summary, emails, etc.)
  // So we make two calls: one for base data, one for extended profile/relationship fields
  const extParams = new URLSearchParams(baseParams);
  ['profile.positions', 'profile.education', 'profile.headline',
   'relationships.billings', 'relationships.assignments', 'relationships.candidatesLists'
  ].forEach(f => extParams.append('fields[]', f));

  const [baseRes, extRes] = await Promise.all([
    ezekiaFetch(`/api/people?${baseParams}`),
    ezekiaFetch(`/api/people?${extParams}`),
  ]);

  // Merge by index — both calls return same page in same order
  const basePeople = baseRes.data || [];
  const extPeople = extRes.data || [];

  const merged = basePeople.map((base, i) => {
    const ext = extPeople[i] || {};
    return {
      ...base,
      profile: { ...base.profile, ...ext.profile },
    };
  });

  return { ...baseRes, data: merged };
}
...
async function transformPerson(ezekiaPerson, db) {
  ...
  // Extract skills from all positions — deduplicated
  ...
  return {
    ...
    headline: ezekiaPerson.profile?.headline || (currentPosition ? `${currentPosition.title}${currentPosition.company?.name ? ' at ' + currentPosition.company.name : ''}` : null),
    bio: ezekiaPerson.summary || (currentPosition?.summary ? currentPosition.summary : null),
    ...
    expertise_tags: allSkills.size > 0 ? [...allSkills].slice(0, 30) : (ezekiaPerson.tags?.map(t => t.name || t) || []),
    industries: industries.length > 0 ? industries.slice(0, 10) : [],
    ezekia_data: {
      id: ezekiaPerson.id,
