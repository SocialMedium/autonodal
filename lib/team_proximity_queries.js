 ...
 *
 * PIPELINE-CONTEXT: These functions accept a pool/db parameter from the caller.
 * Currently only called from scripts/ (pipeline context). The caller is responsible
 * for passing a tenant-scoped db client (TenantDB) or platformPool as appropriate.
 * Safe because: all queries include explicit tenant_id parameters.
 */
...
async function getConsultantPlacementHistory(pool, userId, limitMonths = null) {
  ...
  const result = await pool.query(`
    ...
    JOIN accounts c ON pl.client_id = c.id
...
async function getConsultantLeaderboard(pool, periodMonths = 12, limit = 10) {
  const result = await pool.query(`
    ...
    FROM users u
    LEFT JOIN conversions pl ON u.id = pl.placed_by_user_id
      AND pl.start_date > NOW() - INTERVAL '${periodMonths} months'
  ...
  const result = await pool.query(`
    SELECT 
      ...
      CASE 
        ...
        ELSE 'at_risk'
      END as client_tier
    FROM account_financials cf
    JOIN accounts c ON cf.client_id = c.id
...
async function getTopClients(pool, limit = 20) {
  const result = await pool.query(`
    SELECT 
      ...
      END as tier
    FROM accounts c
    JOIN account_financials cf ON c.id = cf.client_id
