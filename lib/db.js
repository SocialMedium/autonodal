...
// Falls back to DATABASE_URL if DATABASE_URL_APP not set (dev/migration)
const pool = new Pool({
...
// Admin pool — superuser (postgres), for migration scripts ONLY
// Never use in request handlers or services
...
// Log first connection only — suppress per-connection spam in production
let _dbConnected = false;
pool.on('connect', () => {
  if (!_dbConnected) { console.log('📦 Database connected'); _dbConnected = true; }
});
