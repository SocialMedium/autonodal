...
function preparePersonText(person) {
  ...
  // Add investor metadata if available
  if (person.is_investor) {
    const invParts = ['Investor'];
    if (person.investor_type) invParts.push(`Type: ${person.investor_type}`);
    if (person.investor_stage_focus) invParts.push(`Stage: ${person.investor_stage_focus}`);
    if (person.investor_sector_focus) invParts.push(`Sectors: ${person.investor_sector_focus}`);
    if (person.investor_geo_focus) invParts.push(`Geography: ${person.investor_geo_focus}`);
    parts.push(invParts.join(', '));
  }
