...
const TIMEOUT_MS = 15000; // publications collection is 2M+ vectors — allow longer
...
function qdrantPublicationsSearch(vector, limit, filter = null, scoreThreshold = null) {
  return new Promise((resolve, reject) => {
    ...
    const body = {
      ...
      params: { hnsw_ef: 64 }, // lower ef = faster search on 2M+ collection
    };
...
function mapPublication(point) {
  ...
  // Actual payload: title, abstract, first_author, repository, keywords (string), year, date
  // Also handle potential future fields: authors, doi, has_industry_coauthor, citation_count

  // Normalize authors — first_author is primary field, authors is fallback
  let authors = p.authors || p.author || p.first_author || p.creator || '';
  ...
  // Normalize subjects/keywords — publications collection uses comma-separated string
  let subjects = p.subjects || p.keywords || p.tags || p.categories || [];
  if (typeof subjects === 'string') subjects = subjects.split(/[,;]\s*/).map(s => s.trim()).filter(Boolean);

  // Normalize year — publications have integer year + ISO date string
  const year = p.year || (p.date ? new Date(p.date).getFullYear() : null)
    || (p.published_date ? parseInt(p.published_date) : null)
  ...
  // DOI (not in current payload but may be added)
  ...
  return {
    ...
    year,
    source: p.repository || p.source || p.publisher || p.journal || null,
    doi,
    doi_url: doiUrl,
