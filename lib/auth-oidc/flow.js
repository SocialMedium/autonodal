...
async function exchangeCode(provider, code, codeVerifier, redirectUri) {
  ...
  // Diagnostic log (does NOT expose secret — just length + prefix)
  console.log(`[OIDC ${provider.id}] exchangeCode — client_id: ${clientId ? clientId.slice(0, 6) + '...' + clientId.slice(-4) : 'MISSING'} (${clientId?.length || 0} chars), redirect_uri: ${uri}, secret: ${clientSecret ? clientSecret.slice(0, 8) + '... (' + clientSecret.length + ' chars)' : 'MISSING'}, pkce: ${codeVerifier ? 'yes' : 'no'}`);
  ...
  console.log(`[OIDC ${provider.id}] token endpoint status ${res.status}, response keys: ${Object.keys(data).join(',')}, error: ${data.error || 'none'}, description: ${data.error_description || 'none'}`);
