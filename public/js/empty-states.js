...
var EMPTY_STATES = {
  ...
  'events-section': { icon: '\ud83d\udcc5', title: 'No upcoming events in your markets', body: 'Events matching your sectors and geographies appear here \u2014 the only forward-looking signal type.', action: { label: 'Browse events on EventMedium', href: 'https://www.eventmedium.ai/events.html', target: '_blank' } },
...
function renderEmptyState(sectionId, containerId) {
  ...
  if (state.action) {
    actionHtml = state.action.href
      ? '<a href="' + state.action.href + '"' + (state.action.target ? ' target="' + state.action.target + '" rel="noopener noreferrer"' : '') + ' class="empty-action">' + state.action.label + ' \u2192</a>'
