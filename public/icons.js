// icons.js — Lucide-based SVG icon system
// Replaces emoji icons with clean Lucide SVGs
// Usage: icon('trending-up') or signalIcon('capital_raising')
...
const LUCIDE_ICONS = {
  ...
  'git-merge': '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M6 21V9a9 9 0 0 0 9 9"/></svg>',
  ...
  'user-minus': '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="22" x2="16" y1="11" y2="11"/></svg>',
  ...
  'bar-chart': '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="20" y2="10"/><line x1="18" x2="18" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="16"/></svg>',
...
const SIGNAL_ICON_MAP = {
  capital_raising: 'trending-up',
  ma_activity: 'git-merge',
  product_launch: 'rocket',
  partnership: 'handshake',
  geographic_expansion: 'globe',
  restructuring: 'settings',
  leadership_change: 'user',
  layoffs: 'user-minus',
  strategic_hiring: 'users',
};

// Interaction type → Lucide icon name mapping
const INTERACTION_ICON_MAP = {
  ...
  sms: 'message-square',
  note: 'clipboard',
  research_note: 'clipboard',
};

function icon(name, size) {
  var s = size || 16;
  var svg = LUCIDE_ICONS[name];
  if (!svg) return '';
  if (s !== 16) svg = svg.replace(/width="16"/g, 'width="' + s + '"').replace(/height="16"/g, 'height="' + s + '"');
  return svg;
}

function signalIcon(signalType, size) {
  return icon(SIGNAL_ICON_MAP[signalType] || 'zap', size);
}
